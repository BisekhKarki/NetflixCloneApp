...
import "../assets/style/NewPopular.css"
...
const NewAndPopular = () => {
    ...
    useEffect(()=>{
        ...
        const tv = async ()=>{
            ...
            const movieLists = `https://api.themoviedb.org/3/tv/on_the_air?api_key=${apiKey}&page=${2}`
            try{    
                let tvData = await fetch(movieLists);
                let response = await tvData.json();
                setTvLength(response.results.length)
                setNewAndPopular(response.results)
